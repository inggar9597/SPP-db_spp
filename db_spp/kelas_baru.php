...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		if(($submit=='simpan') AND isset($_REQUEST['nis'])){
			$nis = $_REQUEST['nis'];
			$sql = mysql_query("INSERT INTO tb_kelas VALUES('$kelas','$tapel','$nis')");
		}
		
		//proses hapus siswa dari kelas
		if(($submit=='hapus') AND isset($_REQUEST['nis'])){
			$nis = $_REQUEST['nis'];
			$qsiswa = mysql_query("DELETE FROM tb_kelas WHERE kelas='$kelas' AND th_pelajaran='$tapel' AND nis='$nis'");
		}
		...
		echo '<input type="hidden" name="kd_jurusan" value="'.$kd_jurusan.'">';
		...
		$qsiswa = mysql_query("SELECT nis,nm_siswa FROM tb_siswa WHERE kd_jurusan='$kd_jurusan' AND nis NOT IN (SELECT nis FROM tb_kelas ) ORDER BY nis");
		while(list($nis,$nm_siswa)=mysql_fetch_array($qsiswa)){
			echo '<option value="'.$nis.'">'.$nis.' '.$nm_siswa.'</option>';
		}
		echo '</select></div>';
		...
		$qkelas = mysql_query("SELECT nis FROM tb_kelas WHERE kelas='$kelas' AND th_pelajaran='$tapel'");
		if(mysql_num_rows($qkelas) > 0){
			$no=1;
			while(list($nis)=mysql_fetch_array($qkelas)){
				echo '<tr><td>'.$no.'</td>';
				echo '<td>'.$nis.'</td>';
				$qsiswa = mysql_query("SELECT nm_siswa FROM tb_siswa WHERE nis='$nis'");
				list($siswa) = mysql_fetch_array($qsiswa);
				echo '<td>'.$siswa.'</td>';
				//button hapus siswa
				echo '<td><a href="admin.php?hlm=master&sub=kelas&aksi=view&nis='.$nis.'&kelas='.$kelas.'&tapel='.$tapel.'&kd_jurusan='.$kd_jurusan.'&submit=hapus" class="btn btn-danger btn-xs">Hapus</a></td></tr>';
				$no++;
			}
		} else {
...
<form method="post" action="admin.php?hlm=master&sub=kelas&aksi=view" class="form-horizontal" role="form">
	...
	<div class="form-group">
		...
		<div class="col-sm-3">
			<select class="form-control" id="nm_jurusan" name="kd_jurusan">
			<?php
