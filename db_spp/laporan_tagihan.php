...
if( empty( $_SESSION['iduser'] ) ){
   ...
   $sql = mysql_query("SELECT s.nis,s.nm_siswa,k.kelas,p.bulan,p.jumlah FROM (tb_siswa s INNER JOIN tb_kelas k ON s.nis = k.nis) LEFT JOIN tb_transaksi p ON s.nis = p.nis ORDER BY k.kelas, s.nis");
   ...
   $no=1;
   while(list($nis,$nm_siswa,$kls,$bln,$jml)=mysql_fetch_array($sql)){
      echo '<tr><td>'.$no.'</td><td>'.$nis.'</td><td>'.$nm_siswa.'</td><td>'.$kls.'</td>';
